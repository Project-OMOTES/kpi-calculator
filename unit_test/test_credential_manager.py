...
from src.kpicalculator.security.credential_manager import (
    CredentialManager, SecureCredentialManager, ConfigFileCredentialManager,
    ChainedCredentialManager, create_default_credential_manager
)
...
class TestSecureCredentialManager(unittest.TestCase):
    ...
    def test_get_database_credentials_defaults(self):
        ...
        self.assertIsNotNone(credentials)
        self.assertFalse(credentials.ssl)  # Default from code: ssl_env.lower() in ("true", "1", "yes", "on")
        self.assertFalse(credentials.verify_ssl)  # Default from code: verify_ssl_env.lower() in ("true", "1", "yes", "on")
        self.assertEqual(credentials.database, "energy_profiles")  # Default from code: os.getenv(..., "energy_profiles")
    ...
    def test_get_database_credentials_not_found(self):
        ...
        clean_env = {k: v for k, v in os.environ.items() 
                    if not k.startswith('KPI_DB_NONEXISTENT_COM_443_')}
    ...
    def test_get_database_credentials_empty_username(self):
        ...
        test_env = {k: v for k, v in os.environ.items() 
                   if not k.startswith('KPI_DB_EXAMPLE_COM_443_')}
        test_env.update({
            'KPI_DB_EXAMPLE_COM_443_USERNAME': '',
            'KPI_DB_EXAMPLE_COM_443_PASSWORD': 'testpass'
        })
    ...
    def test_get_database_credentials_empty_password(self):
        ...
        test_env.update({
            'KPI_DB_EXAMPLE_COM_443_USERNAME': 'testuser',
            'KPI_DB_EXAMPLE_COM_443_PASSWORD': ''
        })
...
class TestConfigFileCredentialManager(unittest.TestCase):
    ...
    def test_validate_file_permissions_secure(self):
        ...
        mock_stat = Mock()
        mock_stat.st_mode = 0o100600  # Regular file with 600 permissions
        
        with patch.object(self.config_path, 'stat', return_value=mock_stat):
    ...
    def test_validate_file_permissions_insecure(self):
        ...
        mock_stat = Mock()
        mock_stat.st_mode = 0o100644  # Regular file with 644 permissions (world readable)
    ...
    def test_validate_file_permissions_windows(self):
        ...
        with patch('hasattr', side_effect=lambda obj, attr: attr not in ['S_IRGRP', 'S_IROTH']):
...
class TestChainedCredentialManager(unittest.TestCase):
    ...
    def test_get_database_credentials_first_manager_success(self):
        ...
        test_credentials = DatabaseCredentials(
            host="example.com", port=443, username="user", password="pass",
            database="db", ssl=True, verify_ssl=True
        )
...
class TestCredentialManagerErrorHandling(unittest.TestCase):
    ...
    def test_secure_manager_with_malformed_environment(self):
        ...
        test_env = {k: v for k, v in os.environ.items() 
                   if not k.startswith('KPI_DB_MALFORMED_')}
        test_env.update({
