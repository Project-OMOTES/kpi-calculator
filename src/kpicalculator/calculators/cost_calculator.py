...
class CostCalculator:
    ...
    def calculate_npv(self, system_lifetime: int, discount_rate: float = 5.0) -> float:
        ...
        for asset in self.energy_system.assets:
            ...
            opex_npv = opex_annual * sum([
                1.0 / math.pow(1.0 + discount_rate / 100.0, n)
    ...
    def calculate_lcoe(self, system_lifetime: int, discount_rate: float = 5.0) -> float:
        ...
        annual_energy = energy_calc.get_total_energy_consumption_per_year() / 3.6e9  # Convert to MWh
    ...
    def _calculate_capex_for_category(self, category: str) -> float:
        ...
        for asset in self.energy_system.assets:
            if category == "All" or self._asset_belongs_to_category(asset, category):
                capex += self._calculate_investment_cost(asset) + self._calculate_installation_cost(asset)
