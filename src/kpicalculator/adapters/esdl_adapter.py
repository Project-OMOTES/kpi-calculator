...
class EsdlAdapter:
    ...
    def load(self, esdl_file: str, time_series_file: str, 
             pipes_cost_file: str, assets_cost_file: str) -> EnergySystem:
    ...
    def _create_asset_from_esdl(self, esdl_element: Any, time_series_dict: Any,
                               pipe_costs: pd.DataFrame, asset_costs: pd.DataFrame,
                               model_name: str) -> Optional[Asset]:
        ...
        try:
            ...
            asset_dict.update({
                ...
                'discount_rate': float(cost_row['discountRate']) if 'discountRate' in cost_row else 5.0,
            })
    ...
    def _get_power(self, esdl_element: Any) -> float:
        ...
        """
        if ((isinstance(esdl_element, esdl.Producer)) or
                (isinstance(esdl_element, esdl.Consumer)) or
                (isinstance(esdl_element, esdl.Conversion))):
