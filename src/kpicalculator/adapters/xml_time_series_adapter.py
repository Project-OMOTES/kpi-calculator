...
class PiXmlTimeSeries:
    def __init__(self, time_series_xml_file, name_at, property_at, remove_name=True):
        ...
        if not 'series' in xml_dict['TimeSeries']:
            return
        # if there is only one time series or multiple.
        if 'header' in xml_dict['TimeSeries']['series']:
            ...
            time_series.parse_existing(xml_dict['TimeSeries']['series']['header'], name_at, property_at)
    ...
    def add_timer_series(self, pi_xml_type, location_id, parameter_id, qualifier_id, time_step, start_date, end_date,
                         forecast_date, miss_val, station_name, lat, lon, x, y, z, units, creation_date, creation_time):
        time_series = TimeSeries(pi_xml_type, location_id, parameter_id, qualifier_id, time_step, start_date, end_date,
                                 forecast_date, miss_val, station_name, lat, lon, x, y, z, units, creation_date,
                                 creation_time)
            ...
            for prop in self.time_series[name]:
                if self.time_series[name][prop].new_element:
                    ...
                    for series_element in root.iter('{http://www.wldelft.nl/fews/PI}series'):
                        name_ET = series_element[0].find('{http://www.wldelft.nl/fews/PI}' + self.name_at).text
                        prop_ET = series_element[0].find('{http://www.wldelft.nl/fews/PI}' + self.property_at).text
...
class TimeSeries:
    def __init__(self, pi_xml_type=None, location_id=None, parameter_id=None, qualifier_id=None, time_step=None,
                 start_date=None, end_date=None, forecast_date=None, miss_val=None, station_name=None, lat=None,
                 lon=None, x=None, y=None, z=None, units=None, creation_date=None, creation_time=None, name=None,
                 prop=None):
        self.header_dict = {'type': pi_xml_type,
    ...
    def parse_time_series(self, event_object):
        ...
        if '@date' in event_object:
            self.events.append(PiXmlEvent(event_object['@date'], event_object['@time'],
                                          float(event_object['@value']), int(float(event_object['@flag']))))
        else:
            for event in event_object:
                self.events.append(PiXmlEvent(event['@date'], event['@time'],
                                              float(event['@value']), int(float(event['@flag']))))
        return self.events
    ...
    def get_time_step(self):
        time1 = datetime.datetime.strptime(self.events[0].date + ' ' + self.events[0].time, '%Y-%m-%d %H:%M:%S')
        if len(self.events) > 1:
            time2 = datetime.datetime.strptime(self.events[1].date + ' ' + self.events[1].time, '%Y-%m-%d %H:%M:%S')
        else:
...
class PiXmlEvent:
    ...
    def save_event(self, element):
        if self.new_event:
            ...
            event_element.attrib = {'date': str(self.date),
                                    'flag': str(self.flag),
                                    'time': str(self.time),
...
def main():
    ...
    timeseries = pixml_object.add_timer_series('continuous', 'somewhere', 'something', 'OK', 200, '20-10-2020',
                                               '21-10-2020',
                                               '20-10-2020', -999, 'hello', 0, 0, 10, -10, 50, 'm3/h', '20-10-2020',
                                               '10:00')
